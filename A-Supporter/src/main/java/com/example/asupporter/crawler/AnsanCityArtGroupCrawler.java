...
@Controller
public class AnsanCityArtGroupCrawler {
    ...
    public void ansanCityArtGroupCrawler() {
    	try {
	    	String originalURL = "https://ansanarts.com/?page_id=3331";
	        ...
	        for(Element elements : element) {
				...
				if (referenceDate.compareTo(postedDate) <= 0) { // 기준일 이후일 때에는 데이터를 저장한다.
					...
					String url = elements.getElementsByAttribute("href").attr("href");
					dto.setCname("안산시립예술단");
		            dto.setTitle(title);
		            dto.setUrl(url);
	        ...
	        System.out.println("*-----------------------------------------*");
            System.out.println("|      안산시립예술단     |"+"         " +count+"          |");
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
