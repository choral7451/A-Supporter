...
@Controller
public class SeoulCityArtGroupCrawler {
    ...
    public void seoulCityArtGroupCrawler() {
    	try {
	    	String originalURL = "https://www.sjartgroups.or.kr/sjartgroups/bbs/B0000025/list.do?menuNo=300056";
	        ...
	        for(Element elements : element) {
				String postedDate = elements.select("td:nth-child(2)").text().substring(4).replace(".", "-");
				if (referenceDate.compareTo(postedDate) <= 0) { // 기준일 이후일 때에는 데이터를 저장한다.
					String title = elements.select("td:nth-child(1)").text();
					String tmpUrlFirst = "https://www.sjartgroups.or.kr";
	        ...
	        System.out.println("*-----------------------------------------*");
            System.out.println("|      서울시예술단     |"+"         " +count+"          |");
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
    }
